...
from helper.telegram_api import send_message, set_webhook, set_menu_commands
from helper.utils import process_telegram_data, generate_text_response, generate_image_response, generate_file_response
    ...
    if request.is_json:
        ...
        try:
            ...
            if telegram_data['is_text']:
                ...
                return 'OK', 200
            if telegram_data['is_photo']:
                response = generate_image_response(telegram_data['file_id'])
                send_message(telegram_data['sender_id'], response)
                return 'OK', 200
            if telegram_data['is_document']:
                response = generate_file_response(telegram_data['file_id'], telegram_data['mime_type'])
                send_message(telegram_data['sender_id'], response)
                return 'OK', 200
        except:
            pass
